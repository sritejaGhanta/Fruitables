...
if (extraConfig.table_name === 'mod_group_master') {
  ...
  if (!restrictedGroups.includes(inputParams.group_code)) {
    ...
    queryObject.andWhere(`${extraConfig.table_alias}.groupCode IN (:...restrictedGroups)`, { restrictedGroups: restrictedGroups });
    // if (whereClause) {
  ...
  prepareListingCriteria(searchObj, inputParams, aliasList, queryObject) {
    const recLimit = Number(this.getConfigItem('REC_LIMIT')) ? Number(this.getConfigItem('REC_LIMIT')) : 20;
searchObj.page_index = Number(inputParams.page) || Number(inputParams.page_index) || 1;
searchObj.limit = Number(inputParams.limit) ? Number(inputParams.limit) : recLimit;
...
if('filters' in inputParams) {
  if (_.isString(inputParams.filters) && !custom.isEmpty(inputParams.filters)) {
  ...
  if (_.isArray(inputParams.filters)) {
    ...
    filters.forEach(data => {
      ...
      if (aliasKey && aliasKey in aliasList) {
        ...
        queryObject.andWhere(`${aliasList[aliasKey]} = :${prop_val}`, { [prop_val] : aliasVal });
      }
    });
  }
}
...
return tokenInfo;
  ...
  getAdminWhereCriteria(actionType = '', inputParams:any = {}, queryObject, reqObject) {
    let whereClause:any = '';
...
if (actionType) {
  switch (actionType) {
    ...
    case 'update':
       ...
       */
      if (inputParams.status.toLowerCase() === 'inactive' || inputParams.ma_group_id !== inputParams.group_id) {
        if (_.isArray(defaultAdminUsers) && defaultAdminUsers.length > 0) {
    ...
    case 'details':
       ...
       */
      if (_.isArray(restrictAdminGroups) && restrictAdminGroups.length > 0) {
...
Object.keys(inputData).forEach(conKey => {
  ...
  if (replaceVal.substr(0, 8) === "#SYSTEM." && replaceVal.substr(-1) === "#") {
...
return dataObject;
  ...
  if (_.isArray(inputParams.sort)) {
    const sortList = inputParams.sort;
    const orderByColumns = sortList.map((sortField) => {
      let upperSortDir = sortField.dir ? sortField.dir.toUpperCase() : 'ASC';
      
      if (sortField.prop && upperSortDir && aliasList[sortField.prop] && ['ASC', 'DESC'].indexOf(upperSortDir) !== -1 ) {
