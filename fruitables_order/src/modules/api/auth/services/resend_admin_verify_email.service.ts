...
@Injectable()
export class ResendAdminVerifyEmailService extends BaseService {
  
  
  protected readonly log = new LoggerHandler(
    ResendAdminVerifyEmailService.name,
  ).getInstance();
  ...
  constructor() {
    ...
    this.singleKeys = [
      ...
      'update_email_verify_code',
    ];
  }

  /**
   * startResendAdminVerifyEmail method is used to initiate api execution flow.
   * @param array reqObject object is used for input request.
   * @param array reqParams array is used for input params.
   * @param array reqFiles array is used for post files.
   * @return array outputResponse returns output response of API.
   */
  async startResendAdminVerifyEmail(reqObject, reqParams) {
    let outputResponse = {};
    ...
    try {
      ...
      inputParams = await this.getAdminDataById(inputParams);
      if (!_.isEmpty(inputParams.get_admin_data_by_id)) {
      inputParams = await this.prepareVerifyEmailLink(inputParams);
      inputParams = await this.updateEmailVerifyCode(inputParams);
      ...
      this.log.error('API Error >> resend_admin_verify_email >>', err);
    }
  ...
  /**
   * getAdminDataById method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async getAdminDataById(inputParams: any) {
    this.blockResult = {};
    try {
    ...
    inputParams.get_admin_data_by_id = this.blockResult.data;
  ...
  /**
   * updateEmailVerifyCode method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async updateEmailVerifyCode(inputParams: any) {
    this.blockResult = {};
    try {                
    ...
    inputParams.update_email_verify_code = this.blockResult.data;
    inputParams = this.response.assignSingleRecord(
      inputParams,
      this.blockResult.data,
    );
  ...
  finishSuccess(inputParams: any) {
    ...
    return this.response.outputResponse(
      ...
      {
        name: 'resend_admin_verify_email',
      },
    );
  }
