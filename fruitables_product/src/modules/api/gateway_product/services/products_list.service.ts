  ...
  async getProductsList(inputParams: any) {
    ...
    try {
      ...
      if (!custom.isEmpty(inputParams.keyword)) {
        queryObject.orWhere('p.vProductName LIKE :vProductName', { vProductName: `${inputParams.keyword}%` });
      }
      if (!custom.isEmpty(inputParams.keyword)) {
        queryObject.orWhere('p.fProductCost = :fProductCost', { fProductCost: inputParams.keyword });
      }
      if (!custom.isEmpty(inputParams.keyword)) {
        queryObject.orWhere('p.eStatus = :eStatus', { eStatus: inputParams.keyword });
      }
      //@ts-ignore;              
