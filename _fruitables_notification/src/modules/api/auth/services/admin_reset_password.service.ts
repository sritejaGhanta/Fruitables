...
@Injectable()
export class AdminResetPasswordService extends BaseService {
  
  
  protected readonly log = new LoggerHandler(
    AdminResetPasswordService.name,
  ).getInstance();
  ...
  /**
   * startAdminResetPassword method is used to initiate api execution flow.
   * @param array reqObject object is used for input request.
   * @param array reqParams array is used for input params.
   * @param array reqFiles array is used for post files.
   * @return array outputResponse returns output response of API.
   */
  async startAdminResetPassword(reqObject, reqParams) {
    let outputResponse = {};
    ...
    try {
      ...
      inputParams = await this.getAdminInfo(inputParams);
      if (!_.isEmpty(inputParams.get_admin_info) && inputParams.ma_otp_code === inputParams.otp_code) {
      inputParams = await this.resetAdminPassword(inputParams);
      ...
      this.log.error('API Error >> admin_reset_password >>', err);
    }
  ...
  /**
   * getAdminInfo method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async getAdminInfo(inputParams: any) {
    this.blockResult = {};
    try {
      ...
      queryObject.addSelect('otpCode', 'ma_otp_code');
      if (!custom.isEmpty(inputParams.email)) {
    ...
    inputParams.get_admin_info = this.blockResult.data;
  ...
  /**
   * resetAdminPassword method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async resetAdminPassword(inputParams: any) {
    this.blockResult = {};
    try {                
    ...
    inputParams.reset_admin_password = this.blockResult.data;
    inputParams = this.response.assignSingleRecord(
      inputParams,
      this.blockResult.data,
    );
  ...
  tokenFailure(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 1,
      message: custom.lang('Oops! Token is invalid. Please check with valid url.'),
      fields: [],
    };
    return this.response.outputResponse(
      {
