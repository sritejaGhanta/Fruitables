...
@Injectable()
export class AdminChangeStatusService extends BaseService {
  
  
  protected readonly log = new LoggerHandler(
    AdminChangeStatusService.name,
  ).getInstance();
  ...
  /**
   * startAdminChangeStatus method is used to initiate api execution flow.
   * @param array reqObject object is used for input request.
   * @param array reqParams array is used for input params.
   * @param array reqFiles array is used for post files.
   * @return array outputResponse returns output response of API.
   */
  async startAdminChangeStatus(reqObject, reqParams) {
    let outputResponse = {};
    ...
    try {
      ...
      inputParams = await this.updateAdminStatus(inputParams);
      if (!_.isEmpty(inputParams.update_admin_status)) {
        outputResponse = this.finishSuccess(inputParams);
      } else {
        outputResponse = this.finishFailure(inputParams);
      }
    } catch (err) {
      this.log.error('API Error >> admin_change_status >>', err);
    }
  ...
  /**
   * updateAdminStatus method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async updateAdminStatus(inputParams: any) {
    this.blockResult = {};
    try {                
    ...
    inputParams.update_admin_status = this.blockResult.data;
  ...
  finishSuccess(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 1,
      message: custom.lang('Admin status updated successfully.'),
      fields: [],
    };
