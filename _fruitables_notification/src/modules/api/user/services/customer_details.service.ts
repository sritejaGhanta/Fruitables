...
export class CustomerDetailsService extends BaseService {
  ...
  /**
   * startCustomerDetails method is used to initiate api execution flow.
   * @param array reqObject object is used for input request.
   * @param array reqParams array is used for input params.
   * @param array reqFiles array is used for post files.
   * @return array outputResponse returns output response of API.
   */
  async startCustomerDetails(reqObject, reqParams) {
    let outputResponse = {};
    ...
    try {
      ...
      inputParams = await this.getCustomerDetailsById(inputParams);
      if (!_.isEmpty(inputParams.get_customer_details_by_id)) {
        outputResponse = this.finishSuccess(inputParams);
      } else {
        outputResponse = this.finishFailure(inputParams);
      }
    } catch (err) {
      this.log.error('API Error >> customer_details >>', err);
    }
  ...
  /**
   * getCustomerDetailsById method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async getCustomerDetailsById(inputParams: any) {
    this.blockResult = {};
    try {
    ...
    inputParams.get_customer_details_by_id = this.blockResult.data;
  ...
  finishSuccess(inputParams: any) {
    ...
    const funcData: any = {};
    funcData.name = 'customer_details';
