...
@Injectable()
export class UpdateCustomerDetailsService extends BaseService {
  
  
  protected readonly log = new LoggerHandler(
    UpdateCustomerDetailsService.name,
  ).getInstance();
  ...
  /**
   * startUpdateCustomerDetails method is used to initiate api execution flow.
   * @param array reqObject object is used for input request.
   * @param array reqParams array is used for input params.
   * @param array reqFiles array is used for post files.
   * @return array outputResponse returns output response of API.
   */
  async startUpdateCustomerDetails(reqObject, reqParams) {
    let outputResponse = {};
    ...
    try {
      ...
      inputParams = await this.updateCustomerInfo(inputParams);
      if (!_.isEmpty(inputParams.update_customer_info)) {
        outputResponse = this.modCustomerFinishSuccess(inputParams);
      } else {
        outputResponse = this.finishFailure(inputParams);
      }
    } catch (err) {
      this.log.error('API Error >> update_customer_details >>', err);
    }
  ...
  /**
   * updateCustomerInfo method is used to process query block.
   * @param array inputParams inputParams array to process loop flow.
   * @return array inputParams returns modfied input_params array.
   */
  async updateCustomerInfo(inputParams: any) {
    this.blockResult = {};
    try {                
    ...
    inputParams.update_customer_info = this.blockResult.data;
  ...
  modCustomerFinishSuccess(inputParams: any) {
    const settingFields = {
      status: 200,
      success: 1,
      message: custom.lang('Profile details updated successfully.'),
      fields: [],
    };
